...
export default function BookingsPage() {
  ...
  const { data: bookings, isLoading, isError, refetch } = useQuery({ queryKey: ['my-bookings'], queryFn: bookingsApi.listMine })
              ...
              <div className="flex flex-col gap-4 p-5 sm:flex-row sm:items-center">
                ...
                <div className="flex-1">
                  ...
                  <div className="mt-3 flex flex-wrap gap-x-6 gap-y-1 text-sm text-slate-500">
                    <span><span className="font-medium text-slate-700">{new Date(b.startDate).toLocaleDateString()}</span>{' → '}<span className="font-medium text-slate-700">{new Date(b.endDate).toLocaleDateString()}</span></span>
                ...
                <div className="flex flex-col gap-2 sm:w-32">
                  ...
                  {b.status === 'pending_payment' && b.venue && <Link to={`/venues/${b.venue.id}/book`} className="btn-primary text-xs">Pay now</Link>}
                  {b.status !== 'cancelled' && <button onClick={() => handleCancel(b.id)} className="btn-ghost text-xs text-error-600 hover:bg-error-50">Cancel</button>}
                </div>
              </div>
            </div>
          ))}
